...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANkaWBbz2GyYx4OxC02qcVC8EBO_gADfQ5rG1shAVdcPDrbDO19zAEAAwIAA3cAAzgE.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANlaWBcPN0dlFePqKzy4l_k-6Jr8l4AAn4OaxtbIQFXRRPX-stCgXQBAAMCAAN5AAM4BA.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANmaWBdFfrjl0IImpPz_lQq1xqYsogAAoAOaxtbIQFXsbrDZXXLfPYBAAMCAAN5AAM4BA.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANnaWBjKIlj_ms_03oZvW1p6sGYabQAAocOaxtbIQFX87AOKzmjxqgBAAMCAAN3AAM4BA.png)
...
**MAX查询示例：**
1. 将网络构造成从根节点等距的节点环
2. 监听来自下方环的所有消息，然后发送所接收到的所有值的最大值
3. 快速收敛，具有高路径多样性
4. 每个节点只发送一次，成本与树相同
...
2. **f函数**是结合的和交换的：$f(x,y) = f(y,x)$ 且 $f(x,f(y,z)) = f(f(x,y),z)$
...
**ODI属性：**
- 可以融合多个具有相同哈希函数 $h()$ 的FM摘要，通过对摘要进行**按位或运算**
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANpaWBrN1HdYLLv121ejHWhOEkx5SAAArYOaxtbIQFX4PCMO6mO9gkBAAMCAAN3AAM4BA.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANoaWBoGpJgD3abFMZvsqG_m5TVi9UAAqgOaxtbIQFXGtDb2SuYVGABAAMCAAN3AAM4BA.png)

站点1（美国）: $Money=100, Sex=100, Health=94, Mail=92$
站点2（德国）: $Money=20, Sex=15, Mail=5, Health=34$
站点3（日本）: $Money=50, Sex=10, Health=6, Mail=4$

全局: $Money=170, Sex=125, Health=103, Mail=101$
调整因子: $δ_{sex},1=10, δ_{sex},2=-15, δ_{sex},3=5$

**允许误差示例（ε=5）：**

![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANqaWBsxlsHNyxxb_lZoBU1oo8URNUAAroOaxtbIQFX0lyzl26V_PIBAAMCAAN3AAM4BA.png)

站点3触发: $<N3, Health, 41>$
调整: $δ_health,2=2, δ_health,3=-7$
关键技巧: $δhealth,0 = 5$
满足: $δsex,0 + 5 ≥ δ_health,0$
...
布隆过滤器（Bloom Filter，1970年提出）是一种空间高效的概率数据结构，用于**测试某个元素是否属于一个集合。**
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANraWBvKoIxraugk2GgRjnWD-TZFbQAAsoOaxtbIQFXqCsCbziKdb8BAAMCAAN3AAM4BA.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANsaWBvlDA3BliYtPeWRx5vUzRwISwAAs0OaxtbIQFXHzybqnMMeFwBAAMCAAN5AAM4BA.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANtaWBwLNwI32yfCRbR2zRTjqA3Ry8AAs8OaxtbIQFXkFwWEfuPgPQBAAMCAAN3AAM4BA.png)
...
- 不再使用位来标记元素，而是使用一个**计数器**，删除一个元素时即将对应位置的计数减一
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANuaWBx1qDzNODDy0AyD1tJIOUC5UgAAtQOaxtbIQFXo7vQTwABUMMVAQADAgADdwADOAQ.png)
...
![](https://telegraph-image-43w.pages.dev/file/AgACAgUAAyEGAATTNkFKAANvaWByZkBfSbAJYg9DzXhg0P3N_lAAAtcOaxtbIQFXHYQVSOhbRlwBAAMCAAN5AAM4BA.png)
...
设$k = 2$，字符串 $D = \text{abcab}$，转换成一组2-shingle： $$S(D) = {\text{ab}, \text{bc}, \text{ca}}$$
...
1. 最小切割将单个节点孤立后，$vol(B) = 1$ 导致 $ncut$ 值增大，这是最差的划分
