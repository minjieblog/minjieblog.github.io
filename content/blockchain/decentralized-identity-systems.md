...
- **机制**：自动部署 `controller` + `proxy` 两个智能合约；registry合约映射属性；属性哈希存链上，实际数据存IPFS
- **密钥恢复**：社交恢复协议（用户提名受托人，达到投票仲裁后更换公钥）
- **局限**：
    - 可移植性差：仅uPort身份间可互相证明
    - 恶意区块链节点可追踪所有uPortID相关活动
    - 受托人集合可能被攻击，导致身份被永久攻陷

#### Sovrin

- **基础**：开源，基于许可制账本（Indy），仅可信机构（政府/银行/大学）可运行节点
- **特点**：链上不存储Claims；支持点对点通信，无需依赖共享账本
- **局限**：
